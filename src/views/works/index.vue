    ...
    <div class="card-list">
      <transition-group name="bounce">
        <div v-for="(item) in showList" :key="item.title" class="card-item">
          <work-card :title="item.title" :description="item.description" :img="item.img" :type="item.type" :link="item.link"></work-card>
        </div>
      </transition-group>
    </div>
  </div>
...
export default {
  ...
  computed: {
    showList () {
      ...
      return this.list.filter(item => (activeTag === '所有' || activeTag === item.category))
    }
  }
}
</script>
